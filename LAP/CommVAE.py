...
class CommVAE1hot(object):
  ...
  def make_model(self):
    ...
    elif self.obj_fn == "LAP":
        n0 = self.n0 / self.latent_dim
        
        # # Exact KL
        # t1 = 1.0/self.sigma2 * K.sum(K.abs(self.z_mean), axis=-1)
        # t2 = n0/self.sigma2 * K.sum(K.exp(-1.0/n0*K.abs(self.z_mean)))
        # self.kl_loss = t1 + t2
        
        # Upper bound on KL: Source Lemma 2.5, Calibrating Noise to Variance in Adaptive Data Analysis
        t1 = 1.0/(2*self.sigma2*n0) * K.sum(K.square(self.z_mean), axis=-1) 
        t2 = self.latent_dim/7.0 * (self.sigma2**2/n0**2 - 1) * n0**2/self.sigma2**2
        self.kl_loss = t1 + t2
    else:
