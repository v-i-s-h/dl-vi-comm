...
class CommVAE1hot_Aoudia(object):
  def __init__(self, in_dim=None, latent_dim=None, h_dim=None, obj_fn='RBF', n0=1.0, sigma=0.15):
    self.in_dim = in_dim
...
#     self.model.compile( optimizer=tf.train.AdamOptimizer(learning_rate=0.01))
  ...
  def channel( self, faded_blk ):
    ...
    epsilon2 = K.random_normal(shape=K.shape(faded_blk))   # Perturbation at TX output
    # Each component is affected by n0/latent_dim, even pilots.
    return faded_blk + np.sqrt(self.n0/self.latent_dim)*epsilon + self.sigma*epsilon2
