...
parser.add_argument('--sigma2', type=float, help="Prior variance", default=5.0)
...
for i in range(noOfModels):
    ...
    monitors = {
        ...
        "bler_8dB": callbacks.BlerMonitor(m, in_dim=inVecDim, 
                                          ch_use=chUse, snr_dB=8.0, 
                                          interval=100),
        "bler_12dB": callbacks.BlerMonitor(m, in_dim=inVecDim, 
                                          ch_use=chUse, snr_dB=12.0, 
                                          interval=100)
    }
    ...
    noisePower = sym_pow * 10.0**(-GSNR_range_dB/10.0)
    Cg = 1.78
    gamma = 1.0 / np.sqrt(2*Cg) * np.sqrt(noisePower)
    gamma_per_comp = gamma / np.sqrt(2*chUse)
